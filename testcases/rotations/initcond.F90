...
contains
  ...
  ! how to rotate by specific angle instead of arbitrary unit vector?
  subroutine Rotate2(cell)
    type(t_Rbc) :: cell
    real(WP) :: theta, rotmat(3, 3)
    integer :: i, j

    theta = PI / 4.
    rotmat(1, :) = (/1.*COS(theta), -1. * SIN(theta), 0./)
    rotmat(2, :) = (/1.*SIN(theta), -1. * COS(theta), 0./)
    rotmat(3, :) = (/0, 0, 1/)
