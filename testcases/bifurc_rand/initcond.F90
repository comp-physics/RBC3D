program randomized_cell_gen
  ...
  real(WP), parameter :: hematocrit = 0.15
  real(WP) :: tuber = 3
  real(WP), parameter :: tubelen = 14
  ...
  if (rootWorld) write (*, *) "walls(1)%nvert", walls(1)%nvert
  ...
  Lb(3) = maxval(wall%x(:, 3))
  actlen = Lb(3)

  if (rootWorld) write (*, '(A,3F10.3)') 'Periodic domain size = ', Lb

  !Write Walls Out to Wall TecPlot File
  write (fn, FMT=fn_FMT) 'D/', 'wall', 0, '.dat'
  if (rootWorld) print *, "nwall:", nwall
  call WriteManyWalls(fn, nwall, walls)

  print *, "AFTER WriteManyWalls"
